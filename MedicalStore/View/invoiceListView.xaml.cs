...
namespace MedicalStore.View
{
    ...
    public partial class invoiceListView : UserControl
    {
        public invoiceListView()
        {
            ...
            {
                InvoiceDg.ItemsSource = db.Invoices.Include(x=>x.Patient).OrderByDescending(x=>x.CreatedAt).ToList();
            }
        }
        ...
        private void InvoiceDg_OnSelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            using (var db = new ObjectContext())
            {
                InvoiceItemDg.ItemsSource = db.InvoiceItems.Include(x => x.Medicine).Where(x=>x.InvoiceId == selectedItem.Id).ToList();
            }
        }
...
//            if (invoice == null)
//            {
//                MessageBox.Show("Select an invoice", "Inforation", MessageBoxButton.OK,
//                    MessageBoxImage.Information);
//                return;
//            }
//            string[] headers = { "Designation", "Unit of measure", "Unit price", "Quantity", "THT", "TTC" };
...
//            }
        ...
        private void TextBoxBase_OnSelectionChanged(object sender, RoutedEventArgs e)
        {
            ...
            using (var db = new ObjectContext())
            {
//             InvoiceDg.ItemsSource = db.Invoices.Include(x => x.Patient).Where(x=>x.Patient.Name.Contains(term) ||x.Left.ToString(CultureInfo.InvariantCulture).Contains(term)||x.InvoiceNumber.Contains(term)).ToList();
             InvoiceDg.ItemsSource = db.Invoices.Include(x => x.Patient).Where(x=>x.Patient.Name.Contains(term) ||x.InvoiceNumber.Contains(term)).ToList();
            }
        }
